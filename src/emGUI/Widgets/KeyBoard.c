...
static const char cpcLetters[LETERS_ROW_COUNT][MAX_KEY_COUNT + 1] = {"йцукенгшщз:;", "фывапролдж!?", "ячсмитьбюъэх", ".,-"};
static const char cpcLettersUp[LETERS_ROW_COUNT][MAX_KEY_COUNT + 1] = {"ЙЦУКЕНГШЩЗ:;", "ФЫВАПРОЛДЖ!?", "ЯЧСМИТЬБЮЪЭХ", ".,-"};
...
// Letters Keys
  for(int i = 0; i < LETERS_ROW_COUNT; ++i){
    if (i == LETERS_ROW_COUNT - 1) // отступ для клавиши Clear в последнем ряду
      usXR+= usKeyWidth;
